...
class cnews_edit extends cnews {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// date
			$this->date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->date->CurrentValue, 5), ew_CurrentDate(), $this->date->ReadOnly);
...
<textarea data-field="x_details" class="editor" name="x_details" id="x_details" cols="60" rows="6" placeholder="<?php echo ew_HtmlEncode($news->details->PlaceHolder) ?>"<?php echo $news->details->EditAttributes() ?>><?php echo $news->details->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("fnewsedit", "x_details", 60, 6, <?php echo ($news->details->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
ew_CreateCalendar("fnewsedit", "x_date", "%Y-%m-%d");
</script>
<?php } ?>
</span>
