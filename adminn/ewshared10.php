...
class cLanguage {
	...
	function Phrase($Id) {
		...
		if ($ImageUrl <> "") {
			...
			return "<img data-phrase=\"" . $Id . "\" src=\"" . ew_HtmlEncode($ImageUrl) . "\" style=\"border: 0;" . $style . "\" alt=\"" . ew_HtmlEncode($Text) . "\" title=\"" . ew_HtmlEncode($Text) . "\">";
		} else {
			return $Text;
		}
	}
...
class cMenu {
	...
	function Render($ret = FALSE) {
		...
		if (!$this->RenderMenu() && EW_MENUBAR_BRAND == "")
			return;
		if (!$this->IsMobile) {
			if ($this->IsRoot) {
				$str = "<ul";
