...
class ccustomers_edit extends ccustomers {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->image->OldUploadPath = '../includes/upload/customers';
			...
			if (!($this->image->ReadOnly) && !$this->image->Upload->KeepFile) {
				...
				$this->image->ImageWidth = 129; // Resize width
				$this->image->ImageHeight = 129; // Resize height
			}
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (felm && elm && !ew_HasValue(elm))
				return this.OnError(felm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($customers->image->FldCaption()) ?>");
...
?>
<form name="fcustomersedit" id="fcustomersedit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<?php if ($customers->image->Visible) { // image ?>
	<tr id="r_image">
		<td><span id="elh_customers_image"><?php echo $customers->image->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $customers->image->CellAttributes() ?>>
<span id="el_customers_image" class="control-group">
<span id="fd_x_image">
<span class="btn btn-small fileinput-button"<?php if ($customers->image->ReadOnly || $customers->image->Disabled) echo " style=\"display: none;\""; ?>>
	<span><?php echo $Language->Phrase("ChooseFile") ?></span>
	<input type="file" data-field="x_image" name="x_image" id="x_image">
</span>
<input type="hidden" name="fn_x_image" id= "fn_x_image" value="<?php echo $customers->image->Upload->FileName ?>">
...
<?php echo $customers->image->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
