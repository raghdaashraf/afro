...
$services_sub_grid = NULL; // Initialize page object first

class cservices_sub_grid extends cservices_sub {
	...
	// Page object name
	var $PageObjName = 'services_sub_grid';

	// Grid form hidden field names
	var $FormName = 'fservices_subgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["services_sub"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_name") && $objForm->HasValue("o_name") && $this->name->CurrentValue <> $this->name->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_details") && $objForm->HasValue("o_details") && $this->details->CurrentValue <> $this->details->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->id_sub->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->details->setOldValue($objForm->GetValue("o_details"));
		if (!$this->id_sub->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->id_sub->CurrentValue = strval($arKeys[0]); // id_sub
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
