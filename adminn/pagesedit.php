...
class cpages_edit extends cpages {
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->_page->FldIsDetailKey) {
			$this->_page->setFormValue($objForm->GetValue("x__page"));
		}
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->_page->EditValue = ew_HtmlEncode($this->_page->CurrentValue);
			$this->_page->PlaceHolder = ew_RemoveHtml($this->_page->FldCaption());
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->_page->FldIsDetailKey && !is_null($this->_page->FormValue) && $this->_page->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->_page->FldCaption());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// page
			$this->_page->SetDbValueDef($rsnew, $this->_page->CurrentValue, "", $this->_page->ReadOnly);
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($pages->_page->FldCaption()) ?>");
...
?>
<form name="fpagesedit" id="fpagesedit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<?php if ($pages->_page->Visible) { // page ?>
	<tr id="r__page">
		<td><span id="elh_pages__page"><?php echo $pages->_page->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $pages->_page->CellAttributes() ?>>
<span id="el_pages__page" class="control-group">
<input type="text" data-field="x__page" name="x__page" id="x__page" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($pages->_page->PlaceHolder) ?>" value="<?php echo $pages->_page->EditValue ?>"<?php echo $pages->_page->EditAttributes() ?>>
</span>
<?php echo $pages->_page->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<textarea data-field="x_details" class="editor" name="x_details" id="x_details" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($pages->details->PlaceHolder) ?>"<?php echo $pages->details->EditAttributes() ?>><?php echo $pages->details->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("fpagesedit", "x_details", 35, 4, <?php echo ($pages->details->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
<?php echo $pages->details->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
