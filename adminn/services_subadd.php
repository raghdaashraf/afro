...
$services_sub_add = NULL; // Initialize page object first

class cservices_sub_add extends cservices_sub {
	...
	// Page object name
	var $PageObjName = 'services_sub_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_sub"] != "") {
				...
				$this->setKey("id_sub", $this->id_sub->CurrentValue); // Set up key
			} else {
				$this->setKey("id_sub", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("services_sublist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "services_subview.php")
	...
	function AddRow($rsold = NULL) {
		...
		// id_services
		$this->id_services->SetDbValueDef($rsnew, $this->id_services->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$this->id_sub->setDbValue($conn->Insert_ID());
			$rsnew['id_sub'] = $this->id_sub->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($services_sub_add)) $services_sub_add = new cservices_sub_add();

// Page init
$services_sub_add->Page_Init();

// Page main
$services_sub_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$services_sub_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var services_sub_add = new ew_Page("services_sub_add");
services_sub_add.PageID = "add"; // Page ID
var EW_PAGE_ID = services_sub_add.PageID; // For backward compatibility

// Form object
var fservices_subadd = new ew_Form("fservices_subadd");

// Validate form
fservices_subadd.Validate = function() {
...
fservices_subadd.Form_CustomValidate = 
...
fservices_subadd.ValidateRequired = true;
<?php } else { ?>
fservices_subadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fservices_subadd.Lists["x_id_services"] = {"LinkField":"x_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $services_sub_add->ShowPageHeader(); ?>
<?php
$services_sub_add->ShowMessage();
?>
<form name="fservices_subadd" id="fservices_subadd" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="services_sub">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_services_subadd" class="table table-bordered table-striped">
...
<script type="text/javascript">
fservices_subadd.Lists["x_id_services"].Options = <?php echo (is_array($services_sub->id_services->EditValue)) ? ew_ArrayToJson($services_sub->id_services->EditValue, 1) : "[]" ?>;
</script>
<?php } ?>
...
<?php if ($services_sub->details->Visible) { // details ?>
	<tr id="r_details">
		<td><span id="elh_services_sub_details"><?php echo $services_sub->details->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $services_sub->details->CellAttributes() ?>>
<span id="el_services_sub_details" class="control-group">
<textarea data-field="x_details" class="editor" name="x_details" id="x_details" cols="60" rows="6" placeholder="<?php echo ew_HtmlEncode($services_sub->details->PlaceHolder) ?>"<?php echo $services_sub->details->EditAttributes() ?>><?php echo $services_sub->details->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("fservices_subadd", "x_details", 60, 6, <?php echo ($services_sub->details->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<script type="text/javascript">
fservices_subadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$services_sub_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$services_sub_add->Page_Terminate();
?>
