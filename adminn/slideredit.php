...
class cslider_edit extends cslider {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->image->OldUploadPath = '../includes/upload/slider';
			...
			if (!($this->image->ReadOnly) && !$this->image->Upload->KeepFile) {
				...
				$this->image->ImageWidth = 485; // Resize width
				$this->image->ImageHeight = 313; // Resize height
			}
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($slider->title->FldCaption()) ?>");
			felm = this.GetElements("x" + infix + "_image");
			elm = this.GetElements("fn_x" + infix + "_image");
			if (felm && elm && !ew_HasValue(elm))
				return this.OnError(felm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($slider->image->FldCaption()) ?>");
...
?>
<form name="fslideredit" id="fslideredit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="slider">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_slideredit" class="table table-bordered table-striped">
...
<?php echo $slider->id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($slider->id->CurrentValue) ?>">
<?php echo $slider->id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($slider->title->Visible) { // title ?>
	<tr id="r_title">
		<td><span id="elh_slider_title"><?php echo $slider->title->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $slider->title->CellAttributes() ?>>
<span id="el_slider_title" class="control-group">
<textarea data-field="x_title" class="editor" name="x_title" id="x_title" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($slider->title->PlaceHolder) ?>"<?php echo $slider->title->EditAttributes() ?>><?php echo $slider->title->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("fslideredit", "x_title", 35, 4, <?php echo ($slider->title->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
<?php echo $slider->title->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($slider->image->Visible) { // image ?>
	<tr id="r_image">
		<td><span id="elh_slider_image"><?php echo $slider->image->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $slider->image->CellAttributes() ?>>
<span id="el_slider_image" class="control-group">
<span id="fd_x_image">
<span class="btn btn-small fileinput-button"<?php if ($slider->image->ReadOnly || $slider->image->Disabled) echo " style=\"display: none;\""; ?>>
...
<?php echo $slider->image->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
