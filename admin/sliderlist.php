...
class cslider_list extends cslider {
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" href=\"\" onclick=\"ew_SubmitSelected(document.fsliderlist, '" . $this->MultiDeleteUrl . "');return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->IsLoggedIn());
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fsliderlist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// image
			$this->image->UploadPath = '../includes/upload/slider';
			...
			$this->title->EditValue = $this->title->AdvancedSearch->SearchValue;
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_slider\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_slider',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fsliderlist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
...
// Form object
var fsliderlist = new ew_Form("fsliderlist");
...
var fsliderlistsrch = new ew_Form("fsliderlistsrch");

// Validate function for search
fsliderlistsrch.Validate = function(fobj) {
...
// Form_CustomValidate event
fsliderlistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
<?php if (EW_CLIENT_VALIDATE) { ?>
fsliderlistsrch.ValidateRequired = true; // Use JavaScript validation
<?php } else { ?>
fsliderlistsrch.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php } ?>
<?php if ($slider_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $slider_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		...
		if ($slider_list->Recordset = $slider_list->LoadRecordset())
...
<form name="fsliderlistsrch" id="fsliderlistsrch" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>">
<div class="accordion ewDisplayTable ewSearchTable" id="fsliderlistsrch_SearchGroup">
	<div class="accordion-group">
		<div class="accordion-heading">
<a class="accordion-toggle" data-toggle="collapse" data-parent="#fsliderlistsrch_SearchGroup" href="#fsliderlistsrch_SearchBody"><?php echo $Language->Phrase("Search") ?></a>
		</div>
		<div id="fsliderlistsrch_SearchBody" class="accordion-body collapse in">
			<div class="accordion-inner">
<div id="fsliderlistsrch_SearchPanel">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="slider">
<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$slider_list->LoadAdvancedSearch(); // Load advanced search
...
<?php if ($slider->title->Visible) { // title ?>
	<span id="xsc_title" class="ewCell">
		<span class="ewSearchCaption"><?php echo $slider->title->FldCaption() ?></span>
...
<input type="text" data-field="x_title" name="x_title" id="x_title" size="30" maxlength="255" placeholder="<?php echo $slider->title->PlaceHolder ?>" value="<?php echo $slider->title->EditValue ?>"<?php echo $slider->title->EditAttributes() ?>>
<script type="text/javascript">
ew_CreateEditor("fsliderlistsrch", "x_title", 35, 4, <?php echo ($slider->title->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
	</span>
<?php } ?>
...
<div id="xsr_2" class="ewRow">
	...
	<div class="btn-group ewButtonGroup">
	<a class="btn ewShowAll" href="<?php echo $slider_list->PageUrl() ?>cmd=reset"><?php echo $Language->Phrase("ShowAll") ?></a>
	</div>
</div>
</div>
</div>
...
<!--first page button-->
	...
	<a class="btn btn-small" href="<?php echo $slider_list->PageUrl() ?>start=<?php echo $slider_list->Pager->FirstButton->Start ?>"><i class="icon-step-backward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-step-backward"></i></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($slider_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $slider_list->PageUrl() ?>start=<?php echo $slider_list->Pager->PrevButton->Start ?>"><i class="icon-prev"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-prev"></i></a>
	<?php } ?>
<!--current page number-->
	<input class="input-mini" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $slider_list->Pager->CurrentPage ?>">
<!--next page button-->
	<?php if ($slider_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $slider_list->PageUrl() ?>start=<?php echo $slider_list->Pager->NextButton->Start ?>"><i class="icon-play"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-play"></i></a>
	<?php } ?>
<!--last page button-->
	<?php if ($slider_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $slider_list->PageUrl() ?>start=<?php echo $slider_list->Pager->LastButton->Start ?>"><i class="icon-step-forward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-step-forward"></i></a>
	<?php } ?>
</div>
	&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $slider_list->Pager->PageCount ?>
</td>
<td>
	&nbsp;&nbsp;&nbsp;&nbsp;
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $slider_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $slider_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $slider_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($slider_list->SearchWhere == "0=101") { ?>
...
<?php } ?>
<form name="fsliderlist" id="fsliderlist" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="slider">
<div id="gmp_slider" class="ewGridMiddlePanel">
<?php if ($slider_list->TotalRecs > 0) { ?>
<table id="tbl_sliderlist" class="ewTable ewTableSeparate">
...
<?php if ($slider->id->Visible) { // id ?>
	<?php if ($slider->SortUrl($slider->id) == "") { ?>
		<td><div id="elh_slider_id" class="slider_id"><div class="ewTableHeaderCaption"><?php echo $slider->id->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $slider->SortUrl($slider->id) ?>',1);"><div id="elh_slider_id" class="slider_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $slider->id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($slider->id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($slider->id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($slider->title->Visible) { // title ?>
	<?php if ($slider->SortUrl($slider->title) == "") { ?>
		<td><div id="elh_slider_title" class="slider_title"><div class="ewTableHeaderCaption"><?php echo $slider->title->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $slider->SortUrl($slider->title) ?>',1);"><div id="elh_slider_title" class="slider_title">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $slider->title->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($slider->title->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($slider->title->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($slider->image->Visible) { // image ?>
	<?php if ($slider->SortUrl($slider->image) == "") { ?>
		<td><div id="elh_slider_image" class="slider_image"><div class="ewTableHeaderCaption"><?php echo $slider->image->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $slider->SortUrl($slider->image) ?>',1);"><div id="elh_slider_image" class="slider_image">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $slider->image->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($slider->image->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($slider->image->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php
...
<?php echo $slider->id->ListViewValue() ?></span>
<a id="<?php echo $slider_list->PageObjName . "_row_" . $slider_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($slider->title->Visible) { // title ?>
		<td<?php echo $slider->title->CellAttributes() ?>>
<span<?php echo $slider->title->ViewAttributes() ?>>
<?php echo $slider->title->ListViewValue() ?></span>
</td>
	<?php } ?>
	<?php if ($slider->image->Visible) { // image ?>
		<td<?php echo $slider->image->CellAttributes() ?>>
<span>
<?php if ($slider->image->LinkAttributes() <> "") { ?>
<?php if (!empty($slider->image->Upload->DbValue)) { ?>
<img src="<?php echo $slider->image->ListViewValue() ?>" alt=""<?php echo $slider->image->ViewAttributes() ?>>
