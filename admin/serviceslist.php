...
class cservices_list extends cservices {
	...
	function __construct() {
		...
		$this->AddUrl = "servicesadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
	...
	function SetupListOptions() {
		...
		// "detail_services_sub"
		$item = &$this->ListOptions->Add("detail_services_sub");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->IsLoggedIn() && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
		$item->ShowInButtonGroup = FALSE;
		if (!isset($GLOBALS["services_sub_grid"])) $GLOBALS["services_sub_grid"] = new cservices_sub_grid;

		// "detail_success_stories"
		$item = &$this->ListOptions->Add("detail_success_stories");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->IsLoggedIn() && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
		$item->ShowInButtonGroup = FALSE;
		if (!isset($GLOBALS["success_stories_grid"])) $GLOBALS["success_stories_grid"] = new csuccess_stories_grid;
	...
	function RenderListOptions() {
		...
		// "detail_services_sub"
		$oListOpt = &$this->ListOptions->Items["detail_services_sub"];
		if ($Security->IsLoggedIn()) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("services_sub", "TblCaption");
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("services_sublist.php?" . EW_TABLE_SHOW_MASTER . "=services&id=" . strval($this->id->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["services_sub_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
				$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=services_sub")) . "\">" . $Language->Phrase("MasterDetailViewLink") . "</a></li>";
				if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
				$DetailViewTblVar .= "services_sub";
			}
			if ($GLOBALS["services_sub_grid"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
				$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=services_sub")) . "\">" . $Language->Phrase("MasterDetailEditLink") . "</a></li>";
				if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
				$DetailEditTblVar .= "services_sub";
			}
			if ($links <> "") {
		...
		// "detail_success_stories"
		$oListOpt = &$this->ListOptions->Items["detail_success_stories"];
		if ($Security->IsLoggedIn()) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("success_stories", "TblCaption");
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("success_storieslist.php?" . EW_TABLE_SHOW_MASTER . "=services&id=" . strval($this->id->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["success_stories_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
				$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=success_stories")) . "\">" . $Language->Phrase("MasterDetailViewLink") . "</a></li>";
				if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
				$DetailViewTblVar .= "success_stories";
			}
			if ($GLOBALS["success_stories_grid"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
				$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=success_stories")) . "\">" . $Language->Phrase("MasterDetailEditLink") . "</a></li>";
				if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
				$DetailEditTblVar .= "success_stories";
			}
			if ($links <> "") {
				$body .= "<button class=\"btn btn-small dropdown-toggle\" data-toggle=\"dropdown\"><b class=\"caret\"></b></button>";
				$body .= "<ul class=\"dropdown-menu\">". $links . "</ul>";
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_services_sub");
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" href=\"" . ew_HtmlEncode($this->GetAddUrl() . "?" . EW_TABLE_SHOW_DETAIL . "=services_sub") . "\">" . $Language->Phrase("AddLink") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["services_sub"]->TableCaption() . "</a>";
		$item->Visible = ($GLOBALS["services_sub"]->DetailAdd && $Security->IsLoggedIn() && $Security->IsLoggedIn());
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "services_sub";
		}
		$item = &$option->Add("detailadd_success_stories");
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" href=\"" . ew_HtmlEncode($this->GetAddUrl() . "?" . EW_TABLE_SHOW_DETAIL . "=success_stories") . "\">" . $Language->Phrase("AddLink") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["success_stories"]->TableCaption() . "</a>";
		$item->Visible = ($GLOBALS["success_stories"]->DetailAdd && $Security->IsLoggedIn() && $Security->IsLoggedIn());
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "success_stories";
		}
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" href=\"\" onclick=\"ew_SubmitSelected(document.fserviceslist, '" . $this->MultiDeleteUrl . "');return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->IsLoggedIn());

		// Add multi update
		$item = &$option->Add("multiupdate");
		$item->Body = "<a class=\"ewAction ewMultiUpdate\" href=\"\" onclick=\"ew_SubmitSelected(document.fserviceslist, '" . $this->MultiUpdateUrl . "');return false;\">" . $Language->Phrase("UpdateSelectedLink") . "</a>";
		$item->Visible = ($Security->IsLoggedIn());
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_services\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_services',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fserviceslist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
...
<div class="accordion ewDisplayTable ewSearchTable" id="fserviceslistsrch_SearchGroup">
...
<?php if ($services->title->Visible) { // title ?>
	<span id="xsc_title" class="ewCell">
		<span class="ewSearchCaption"><?php echo $services->title->FldCaption() ?></span>
...
<input type="text" data-field="x_title" name="x_title" id="x_title" size="30" maxlength="255" placeholder="<?php echo $services->title->PlaceHolder ?>" value="<?php echo $services->title->EditValue ?>"<?php echo $services->title->EditAttributes() ?>>
</span>
	</span>
<?php } ?>
</div>
...
<!--first page button-->
	...
	<a class="btn btn-small" href="<?php echo $services_list->PageUrl() ?>start=<?php echo $services_list->Pager->FirstButton->Start ?>"><i class="icon-step-backward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-step-backward"></i></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($services_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $services_list->PageUrl() ?>start=<?php echo $services_list->Pager->PrevButton->Start ?>"><i class="icon-prev"></i></a>
	<?php } else { ?>
...
<!--next page button-->
	...
	<a class="btn btn-small" href="<?php echo $services_list->PageUrl() ?>start=<?php echo $services_list->Pager->NextButton->Start ?>"><i class="icon-play"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-play"></i></a>
	<?php } ?>
<!--last page button-->
	<?php if ($services_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $services_list->PageUrl() ?>start=<?php echo $services_list->Pager->LastButton->Start ?>"><i class="icon-step-forward"></i></a>
	<?php } else { ?>
...
?>
<?php if ($services->title->Visible) { // title ?>
	<?php if ($services->SortUrl($services->title) == "") { ?>
		<td><div id="elh_services_title" class="services_title"><div class="ewTableHeaderCaption"><?php echo $services->title->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $services->SortUrl($services->title) ?>',1);"><div id="elh_services_title" class="services_title">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $services->title->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($services->title->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($services->title->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($services->desc->Visible) { // desc ?>
	<?php if ($services->SortUrl($services->desc) == "") { ?>
		<td><div id="elh_services_desc" class="services_desc"><div class="ewTableHeaderCaption"><?php echo $services->desc->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $services->SortUrl($services->desc) ?>',1);"><div id="elh_services_desc" class="services_desc">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $services->desc->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($services->desc->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($services->desc->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php
