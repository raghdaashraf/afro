...
class cusers_list extends cusers {
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" href=\"\" onclick=\"ew_SubmitSelected(document.fuserslist, '" . $this->MultiDeleteUrl . "');return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->IsLoggedIn());
...
<?php if ($users->user_name->Visible) { // user_name ?>
	<?php if ($users->SortUrl($users->user_name) == "") { ?>
		<td><div id="elh_users_user_name" class="users_user_name"><div class="ewTableHeaderCaption"><?php echo $users->user_name->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $users->SortUrl($users->user_name) ?>',1);"><div id="elh_users_user_name" class="users_user_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $users->user_name->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($users->user_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($users->user_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php
