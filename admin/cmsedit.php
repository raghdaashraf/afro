...
class ccms_edit extends ccms {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("cmslist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("cmslist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				...
				if ($this->EditRow()) { // Update record based on key
					...
					if (ew_GetPageName($sReturnUrl) == "cmsview.php")
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($cms->title->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_desc");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($cms->desc->FldCaption()) ?>");
...
?>
<form name="fcmsedit" id="fcmsedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="cms">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_cmsedit" class="table table-bordered table-striped">
<?php if ($cms->title->Visible) { // title ?>
	<tr id="r_title">
		<td><span id="elh_cms_title"><?php echo $cms->title->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<?php echo $cms->title->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($cms->desc->Visible) { // desc ?>
	<tr id="r_desc">
		<td><span id="elh_cms_desc"><?php echo $cms->desc->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $cms->desc->CellAttributes() ?>>
<span id="el_cms_desc" class="control-group">
<textarea data-field="x_desc" class="editor" name="x_desc" id="x_desc" cols="60" rows="4" placeholder="<?php echo $cms->desc->PlaceHolder ?>"<?php echo $cms->desc->EditAttributes() ?>><?php echo $cms->desc->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("fcmsedit", "x_desc", 60, 4, <?php echo ($cms->desc->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
<?php echo $cms->desc->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
