...
$careers_form_cv_blobview = NULL; // Initialize page object first

class ccareers_form_cv_blobview extends ccareers_form {
	...
	// Page object name
	var $PageObjName = 'careers_form_cv_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('careers_form', 'x_cv');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('cv');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('cv')));
				if (trim(strval($this->Recordset->fields('cv'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('cv') . "\"");
				}
...
// Create page object
if (!isset($careers_form_cv_blobview)) $careers_form_cv_blobview = new ccareers_form_cv_blobview();

// Page init
$careers_form_cv_blobview->Page_Init();

// Page main
$careers_form_cv_blobview->Page_Main();
?>
<?php
$careers_form_cv_blobview->Page_Terminate();
?>
