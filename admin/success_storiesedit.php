...
$success_stories_edit = NULL; // Initialize page object first

class csuccess_stories_edit extends csuccess_stories {
	...
	// Page object name
	var $PageObjName = 'success_stories_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("success_storieslist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("success_storieslist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				...
				if ($this->EditRow()) { // Update record based on key
					...
					if (ew_GetPageName($sReturnUrl) == "success_storiesview.php")
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->scope->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->scope->FldCaption()));

			// customer
			$this->customer->EditCustomAttributes = "";
			$this->customer->EditValue = ew_HtmlEncode($this->customer->CurrentValue);
			$this->customer->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->customer->FldCaption()));
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->id_services->FldIsDetailKey && !is_null($this->id_services->FormValue) && $this->id_services->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->id_services->FldCaption());
		}
		if (!$this->scope->FldIsDetailKey && !is_null($this->scope->FormValue) && $this->scope->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// id_services
			$this->id_services->SetDbValueDef($rsnew, $this->id_services->CurrentValue, 0, $this->id_services->ReadOnly);

			// scope
			$this->scope->SetDbValueDef($rsnew, $this->scope->CurrentValue, "", $this->scope->ReadOnly);
...
// Create page object
if (!isset($success_stories_edit)) $success_stories_edit = new csuccess_stories_edit();

// Page init
$success_stories_edit->Page_Init();

// Page main
$success_stories_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$success_stories_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var success_stories_edit = new ew_Page("success_stories_edit");
success_stories_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = success_stories_edit.PageID; // For backward compatibility

// Form object
var fsuccess_storiesedit = new ew_Form("fsuccess_storiesedit");

// Validate form
fsuccess_storiesedit.Validate = function() {
...
fsuccess_storiesedit.Form_CustomValidate = 
...
fsuccess_storiesedit.ValidateRequired = true;
<?php } else { ?>
fsuccess_storiesedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fsuccess_storiesedit.Lists["x_id_services"] = {"LinkField":"x_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $success_stories_edit->ShowPageHeader(); ?>
<?php
$success_stories_edit->ShowMessage();
?>
<form name="fsuccess_storiesedit" id="fsuccess_storiesedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="success_stories">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_success_storiesedit" class="table table-bordered table-striped">
<?php if ($success_stories->id_services->Visible) { // id_services ?>
	<tr id="r_id_services">
		<td><span id="elh_success_stories_id_services"><?php echo $success_stories->id_services->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<script type="text/javascript">
fsuccess_storiesedit.Lists["x_id_services"].Options = <?php echo (is_array($success_stories->id_services->EditValue)) ? ew_ArrayToJson($success_stories->id_services->EditValue, 1) : "[]" ?>;
</script>
<?php } ?>
...
<?php if ($success_stories->scope->Visible) { // scope ?>
	<tr id="r_scope">
		<td><span id="elh_success_stories_scope"><?php echo $success_stories->scope->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $success_stories->scope->CellAttributes() ?>>
<span id="el_success_stories_scope" class="control-group">
<textarea data-field="x_scope" class="editor" name="x_scope" id="x_scope" cols="60" rows="6" placeholder="<?php echo $success_stories->scope->PlaceHolder ?>"<?php echo $success_stories->scope->EditAttributes() ?>><?php echo $success_stories->scope->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("fsuccess_storiesedit", "x_scope", 60, 6, <?php echo ($success_stories->scope->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php if ($success_stories->customer->Visible) { // customer ?>
	<tr id="r_customer">
		<td><span id="elh_success_stories_customer"><?php echo $success_stories->customer->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $success_stories->customer->CellAttributes() ?>>
<span id="el_success_stories_customer" class="control-group">
<input type="text" data-field="x_customer" name="x_customer" id="x_customer" size="30" maxlength="255" placeholder="<?php echo $success_stories->customer->PlaceHolder ?>" value="<?php echo $success_stories->customer->EditValue ?>"<?php echo $success_stories->customer->EditAttributes() ?>>
</span>
...
<?php if ($success_stories->date->Visible) { // date ?>
	<tr id="r_date">
		<td><span id="elh_success_stories_date"><?php echo $success_stories->date->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $success_stories->date->CellAttributes() ?>>
<span id="el_success_stories_date" class="control-group">
<input type="text" data-field="x_date" name="x_date" id="x_date" size="30" maxlength="255" placeholder="<?php echo $success_stories->date->PlaceHolder ?>" value="<?php echo $success_stories->date->EditValue ?>"<?php echo $success_stories->date->EditAttributes() ?>>
</span>
...
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($success_stories->id->CurrentValue) ?>">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("EditBtn") ?></button>
</form>
<script type="text/javascript">
fsuccess_storiesedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$success_stories_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$success_stories_edit->Page_Terminate();
?>
